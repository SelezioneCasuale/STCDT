...
if blnColour
    % full colour version
    clrs = colormap; [rw cl] = size(clrs); clrstep = floor(rw /ngrps);
    % keyboard
else
...
for loop = 1:ngrps
    ...
    if any(plotraster == 2)
        ...
        subplot(nsplots,1,pctr), plot(stgrp,ixorder,'.','Color',clrs(clrstep*loop,:)); hold on;
         pctr = pctr+1;
    end
    
    if any(plotraster == 3)
        subplot(nsplots,1,pctr), plot(stgrp, ixgrp,'.','Color',clrs(clrstep*loop,:)); hold on;
    end
